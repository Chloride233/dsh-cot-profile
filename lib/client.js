  ...
  factory: (require) => {
    ...
    const DICT = {
      'zh-CN': {
        ...
        'panel.mixed': '过渡带',
        'panel.mixed.desc': '判定不稳定：We/The/Let 混合（断裂带）· 置信度 {pct}%',
      ...
      en: {
        ...
        'panel.mixed': 'transition band',
        'panel.mixed.desc': 'unstable: We/The/Let mixing (fault line) · confidence {pct}%',
    ...
    function T(key, params) {
      ...
      const template = DICT['zh-CN'][key] ?? key;
    ...
    function ProfileContent({ profile }) {
      ...
      const verdict = !j.sufficient
        ? [
            React.createElement('div', { key: 'name', style: { ...contentStyles.familyName, color: 'var(--dsw-alias-label-tertiary)' } }, T('panel.sampling')),
            React.createElement('div', { key: 'conf', style: contentStyles.confidence }, T('panel.sampling.desc', { blocks: profile.blocks, minBlocks: profile.minBlocks })),
          ]
        : j.mixed
          ? [
              React.createElement('div', { key: 'name', style: { ...contentStyles.familyName, color: '#d29922' } }, T('panel.mixed')),
              React.createElement('div', { key: 'conf', style: contentStyles.confidence }, T('panel.mixed.desc', { pct })),
            ]
          : [
              React.createElement('div', { key: 'name', style: { ...contentStyles.familyName, color } }, j.family),
              React.createElement('div', { key: 'conf', style: contentStyles.confidence }, T('panel.verdict', { pct, blocks: profile.blocks })),
            ];
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement('span', { className: 'cot-prof-pulse' }, j.sufficient ? T('panel.tracking') : T('panel.thinking')),
          ),
        ),
        ...
        React.createElement(
          ...
          metric(T('panel.we'), c.we),
          metric(T('panel.letMe'), c.letMe),
          metric(T('panel.lets'), c.lets),
          metric(T('panel.i'), c.i),
          metric(T('panel.p50'), profile.p50BlockChars + T('panel.p50Unit')),
          metric(T('panel.replies'), profile.visibleReplies),
        ),
        React.createElement('div', { style: contentStyles.sectionLabel }, T('panel.firstLines')),
        React.createElement(
          'div',
          { style: contentStyles.grid },
          metric(T('panel.flWeNeed'), fl['we-need']),
          metric(T('panel.flUserWants'), fl['the-user-wants']),
          metric(T('panel.flLetMe'), fl['let-me']),
          metric(T('panel.flI'), fl.i),
        ),
