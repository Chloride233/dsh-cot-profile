  ...
  factory: (require) => {
    ...
    const DICT = {
      'zh': {
        ...
        'panel.side.spec': 'spec 侧',
        'panel.side.react': 'react 侧',
        'panel.side.gray': '灰测轨迹',
        'panel.voice.collective': '集体口吻（we 主导）',
        'panel.voice.doer': '第一人称执行（let me 主导）',
        'panel.voice.mixed': '混合口吻',
        'panel.voice.neutral': '口吻中性',
      ...
      en: {
        ...
        'panel.side.spec': 'spec side',
        'panel.side.react': 'react side',
        'panel.side.gray': 'gray trajectory',
        'panel.voice.collective': 'collective voice (we-led)',
        'panel.voice.doer': 'first-person doer (let me-led)',
        'panel.voice.mixed': 'mixed voice',
        'panel.voice.neutral': 'neutral voice',
        'panel.axis.spec': 'spec',
        'panel.axis.react': 'react',
    ...
    function ProfileContent({ profile }) {
      ...
      // Voice semantics (the readable version of the raw counts).
      let voice = 'neutral';
      if (we > 0 || letMe > 0) {
        if (we > letMe * 3) voice = 'collective';
        else if (letMe > we * 3) voice = 'doer';
        else voice = 'mixed';
      }
      const voiceKey = {
        collective: 'panel.voice.collective',
        doer: 'panel.voice.doer',
        mixed: 'panel.voice.mixed',
        neutral: 'panel.voice.neutral',
      }[voice];
      ...
      const verdict = !j.sufficient
        ...
        : React.createElement(
            ...
            React.createElement(
              ...
              React.createElement('div', { style: { ...contentStyles.familyName, color } }, j.mixed ? T('panel.mixed') : T('panel.side.' + (j.family === 'minimal-like' ? 'spec' : j.family === 'standard-like' ? 'react' : 'gray'))),
              j.mixed ? null : React.createElement('span', { style: { ...contentStyles.confidence, fontSize: 15, fontWeight: 600 } }, j.family),
            ),
            React.createElement(
              'div',
              { style: { display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' } },
              React.createElement('span', { style: contentStyles.confidence }, T('panel.voice.' + voice)),
              React.createElement(
                'span',
                { style: { fontSize: 12, color: 'var(--dsw-alias-label-secondary)', fontVariantNumeric: 'tabular-nums' } },
