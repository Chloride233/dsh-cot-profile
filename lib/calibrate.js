 ...
 */
import { distance, DEFAULT_WEIGHTS } from './analyzer.js';
...
export function baselineDiff(vector, profiles, weights = {}) {
  ...
  const diffs = {};
  for (const dim of Object.keys(best.vector)) {
    const obs = vector[dim] ?? 0;
    const base = best.vector[dim] ?? 0;
    diffs[dim] = Math.round(((obs - base) / (Math.abs(base) + 1)) * 1000) / 1000;
  }
  return { profileId: best.id, distance: Math.round(bestDist * 1000) / 1000, diffs };
}
