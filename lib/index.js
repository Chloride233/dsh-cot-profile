...
import { aggregateRecords, profileFromGroup } from './calibrate.js';
...
const Config = z.object({
  ...
  minBlocksForJudgment: z.number().step(1).min(1).max(500).default(10),
...
const ViewSchema = zz.object({
  ...
  ui: zz.object({ badge: zz.boolean(), panel: zz.boolean(), panelMode: zz.string() }),
...
function apply(ctx, config) {
  ...
  ctx.inject(['webServer'], (webCtx) => {
    ...
    disposeRoute = server.register({
      ...
      handler: (_req, res) => {
        try {
          const file = runtime.config().record.file;
          let records = [];
          if (file) {
          ...
          const groups = aggregateRecords(records).map((group) => ({
            ...group,
            profile: profileFromGroup(group),
          }));
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ file: file || null, total: records.length, groups }));
        } catch (err) {
